    ...
    def __init__(self):
        ...
        try:
            ...
            try:
                ...
                print("🔧 Using multilingual Sentence Transformer embeddings (Hebrew supported)")
            except Exception:
                self.embeddings = SentenceTransformerEmbeddings(
    ...
    def ask(self, question: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        try:
            ...
            if not self.vector_store:
                ...
                if session["memory"] is None or hasattr(session["memory"], "output_key"):
                    ...
                    if session["memory"] and hasattr(session["memory"], "chat_memory") and hasattr(session["memory"].chat_memory, "messages"):
                        direct_memory.chat_memory.messages = session["memory"].chat_memory.messages.copy()
                ...
                if session["memory"] is None or not hasattr(session["memory"], "output_key") or session["memory"].output_key != "answer":
                    ...
                    if session["memory"] and hasattr(session["memory"], "chat_memory") and hasattr(session["memory"].chat_memory, "messages"):
                        rag_memory.chat_memory.messages = session["memory"].chat_memory.messages.copy()
...
Answer in the same language as the question, and provide helpful, accurate information based on the context."""
                ...
                if any(word in question for word in ["הקובץ", "המסמך", "הטקסט", "המידע"]):
                    enhanced_question = f"בהתבסס על המסמך שהועלה, {question}"
                elif "תסביר" in question and ("בעברית" in question or len(question.split()) <= 3):
                    enhanced_question = f"תסביר את התוכן של המסמך שהועלה בעברית"
                elif question.strip() in ["תסביר על הקובץ בבקשה", "תן לי סיכום", "מה יש במסמך"]:
                    enhanced_question = f"בהתבסס על המסמך שהועלה, {question}"
                elif any(word in question.lower() for word in ["the file", "the document", "this document"]):
                    enhanced_question = f"Based on the uploaded document, {question}"
                elif question.lower().strip() in ["explain in english", "summarize this", "what's in this"]:
                    enhanced_question = f"Based on the uploaded document, {question}"
        ...
        if self.documents:
            try:
                ...
                if "quota" in str(e).lower() or "429" in str(e):
                    ...
                    print(f"📥 Downloading sentence-transformers model (first time only)...")
